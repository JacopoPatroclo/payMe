...
import ProductsController from './api/products/ProductsController';
...
const App = new Server(
  ...
  new Api(
    new UserRouter(new UserController(myLogger), authUtil),
    new ProductsRouter(new ProductsController(myLogger, new TicketsManager()))
  ),
  new AuthRouter(new AuthController(myLogger, authUtil), authUtil)
).app;

App.listen(config.PORT, () => {
  console.log(
    `Applicativo avviato sulla porta ${config.PORT} in modalità ${
      config.ENVIROMENT
    }`
  );
});
